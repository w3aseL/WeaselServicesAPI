...
namespace SpotifyAPILibrary.Services
{
    public sealed class SpotifyPlaylistService
    {
        ...
        public async Task<FullPlaylist> CreatePlaylistBySession(SpotifyClient client, int userId, int sessionId)
        {
            var session = _dbLookup.GetSpotifySessionByUser(userId, sessionId);
            var spotifyId = (await client.UserProfile.Current()).Id;

            var newPlaylist = await client.Playlists.Create(spotifyId, new PlaylistCreateRequest($"Session {session.Id}")
            {
                Description = $"The following playlist was generated by Weasel's API Service. This is from session {session.Id}!"
            });

            var tracks = session
                .SpotifyTrackPlays
                .Select(tp => string.Format("spotify:track:{0}", tp.Song.Id))
                .ToList();
